"""
Generates video frames with detected faces, gender, and age information.

The `gen_frames()` function reads frames from a video capture device (webcam) and processes them to detect faces, determine the gender and age of the detected faces, and then generates video frames with the detected information overlaid on the original frames.

The `gen_frames_photo()` function takes an image file, processes it to detect faces, determine the gender and age of the detected faces, and then generates a video frame with the detected information overlaid on the original image.

These functions are used by the Flask web application to provide real-time video processing and image processing capabilities.
"""
